...
export let depTree = logCAFn(
    Level.NOTICE, "Computing dependencies",
    log => async (
        opts: {
        ...
        let [checked_dir, r] = await withDir(
            async ({ path }) => [path, await fstar("Running F*")(log)(
                { ...opts, quiet: "quiet" in opts ? opts.quiet : true },
                "--dep", "raw",
                "--cache_dir", path,
...
export let sort_by_dependencies =
    (log: Logger) =>
        async (opts: {
            ...
            let sorted: string[] = [];
            let deps = await (depTree(log)(opts, ...modules));
            let h = (x: string): void => {
